...
public class Multi_tc extends Thread {
	...
	public Multi_tc(Socket clientSocket, Multi_tc[] threads, ServerSocket serverSocket) {
	...
	public void run() {
		...
		try {
			...
			//output.println("game");
			new Onlychat2("Joao", server).setVisible(true);
			new Onlychat2("Mario", server).setVisible(true);
			
				//new TicTacToeServer().doit(server);
					...
					synchronized (this) {
						for (int i = 0; i < maxClientsCount; i++) {
							if (threads[i] != null
									...
									&& threads[i].clientName.contains("admin")) 
								threads[i].output.println(line);
