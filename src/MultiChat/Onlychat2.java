...
public class Onlychat2 extends JFrame {
	...
	// --- Constructor ---
	public Onlychat2(String s, ServerSocket socket) {
		nick1 = s;
		initUI();
		serverSocket = socket;
	}
		...
		join.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				try {
					...
					conection = new Socket("127.0.0.1", 2222);
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				try{
					...
					new Onlychat2("a", server).setVisible(true);
					new Onlychat2("b", server).setVisible(true);
